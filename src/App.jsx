...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function bootAuth() {
      ...
      setCloudStatus(data?.session?.user ? 'Аккаунт подключён' : 'Локальный режим');
    }
  ...
  useEffect(() => {
    ...
    cloudSaveTimeoutRef.current = setTimeout(async () => {
      setCloudStatus('Сохраняем в облако...');
      const payload = {
        user_id: user.id,
        stats,
        daily_stats: dailyStats,
  ...
  }, [user, cloudHydrated, stats, dailyStats, itemProgress, blockProgress, grammarProgress, archivedItems, mistakes]);
  ...
  async function handleEmailLogin() {
    if (!authEmail.trim()) {
      setCloudStatus('Введи email для входа');
      return;
    }

    setCloudStatus('Отправляем ссылку для входа...');
    ...
    if (error) {
      ...
      setCloudStatus(`Ошибка входа: ${error.message}`);
    } else {
      setCloudStatus('Проверь почту: ссылка для входа отправлена');
    }
  }
        ...
        <div className="grid gap-6 lg:grid-cols-[320px,1fr]">
          <Card className="rounded-3xl border-0">
            ...
            <CardContent className="space-y-5">
              ...
              <div className="space-y-3 rounded-2xl bg-slate-100 p-4">
                <div className="flex items-center gap-2 text-sm font-medium text-slate-700">
                  <UploadCloud className="h-4 w-4" /> Облачный прогресс
                </div>
                {user ? (
                  ...
                  <div className="space-y-3">
                    ...
                    />
                    <Button className="w-full justify-center rounded-2xl" onClick={handleEmailLogin} disabled={authLoading}>
                      Войти по ссылке на email
                    </Button>
                    <div className="text-xs text-slate-500">{cloudStatus}</div>
                  </div>
                )}
              </div>
